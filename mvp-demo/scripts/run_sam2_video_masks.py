        ...
        for frame_idx, object_ids, masks in predictor.propagate_in_video(state):
            ...
            for oi, oid in enumerate(object_ids):
                ...
                if out_path.exists() and not args.overwrite:
                    continue
                # SAM2 returns masks at video resolution as (N, 1, H, W) float scores.
                # OpenCV expects (H, W) or (H, W, C); squeeze the singleton channel dim.
                mask_i = masks[oi]
                if getattr(mask_i, "ndim", None) == 3 and mask_i.shape[0] == 1:
                    mask_i = mask_i[0]
                m = (mask_i > 0).to(torch.uint8).cpu().numpy() * 255
                ok = cv2.imwrite(str(out_path), m)
                if not ok:
                    raise RuntimeError(f"Failed to write mask png: {out_path} (shape={m.shape} dtype={m.dtype})")
