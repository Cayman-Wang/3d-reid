...
def _soft_layer_from_points(
    ...
    alpha_blur = np.clip(alpha_blur, 0.0, 1.0)
    alpha_safe = np.maximum(alpha_blur[..., None], 1e-3)
    layer_rgb = np.clip(color_blur / alpha_safe, 0.0, 255.0).astype(np.uint8)
    return layer_rgb, alpha_blur
...
def _render_camera_overlay_frame(
    ...
    dyn_colors = np.full((dynamic_points.shape[0], 3), np.asarray([255, 120, 60], dtype=np.uint8), dtype=np.uint8)
...
def main() -> None:
    ...
    for orbit_idx, source_idx in enumerate(orbit_source_indices):
        ...
        dynamic_points = _load_point_cloud(_find_point_file(points_dir, row["_points_rel"]))
        if dynamic_points.shape[0] > int(args.max_dynamic_points):
            rng = np.random.default_rng(logical_t_idx * 7919 + 17)
            keep = rng.choice(dynamic_points.shape[0], size=int(args.max_dynamic_points), replace=False)
            keep.sort()
